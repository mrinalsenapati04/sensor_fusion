...
#    print(pts_2d.shape)
    ...
    for i in range(imgfov_pc_pixel.shape[1]):
        ...
        # print('value is ',int(640.0 / depth))
...
#    render_lidar_on_image(pc_velo, rgb, calib, img_width, img_height)
    ...
    basedir='/home/vm/catkin_mrinal/src/sensor_fusion/Dataset/2011_09_26/2011_09_26_drive_0009_sync/image_02/data'
    ...
    for f in sorted(listdir(basedir)) :
        f_path=basedir+'/'+f
        # print(f_path)
        f_split=f_path.split('image_02')
        # print(f_split)
        f_name=f_split[1].split('.png')
        velo_path=f_split[0]+'velodyne_points'+f_name[0]+'.bin'
        project_imgpath= f_split[0]+'projected_image'+f_name[0]+'.jpg'
        image_path=f_path
        # print(velo_path)
        # print(image_path)
        # print(project_imgpath)
        rgb = cv2.cvtColor(cv2.imread(image_path), cv2.COLOR_BGR2RGB)
        img_height, img_width, img_channel = rgb.shape
        pc_velo = load_velo_scan(velo_path)[:, :3]
        img=render_lidar_on_image(pc_velo, rgb, calib, img_width, img_height,proj_velo2cam2)
        cv2.imwrite(project_imgpath, img)
        print('projected image is saved')
        count=count+1
